...
#: Inline-code colour for the rendered Markdown.  GNOME palette orange_1/orange_5;
#: kept in sync with the bundled GtkSourceView schemes in ``styles/`` so that the
#: editor and the preview highlight `code spans` identically.  No GTK named colour
#: fits here, hence the literals.
CODE_FG_DARK = "#FFBE6F"
CODE_FG_LIGHT = "#C64600"
...
<style>
:root {{ --bg: {bg_color}; --fg: {fg_color}; --accent: {accent_color}; --dim: {dim_color}; --card-bg: {card_bg_color}; --borders: {borders_color}; --code-fg: {code_fg_color}; }}
{css_content}
</style>
</head>
<body>
...
class Preview(Gtk.ScrolledWindow):
    ...
    def _get_theme_colors() -> dict[str, str]:
        ...
        def _first_named(names: tuple[str, ...], fallback: Gdk.RGBA) -> str:
            """Return the first of *names* the theme defines."""
            for name in names:
                ok, c = ctx.lookup_color(name)
                if ok:
                    return c.to_string()
        ...
        return {
            ...
            # accent_bg_color is the fill colour for buttons and is too dark to
            # read as link text on a dark background.  accent_color is
            # libadwaita's standalone variant, lightened for exactly this use.
            "accent_color": _first_named(("accent_color", "accent_bg_color"), fg),
            "dim_color": _named("dim_label_color", fg),
            "card_bg_color": _named("card_bg_color", bg),
            "borders_color": _named("borders_color", fg),
            "code_fg_color": (CODE_FG_DARK if Adw.StyleManager.get_default().get_dark()
                              else CODE_FG_LIGHT),
        }
    ...
    def update_theme(self) -> None:
        ...
        js = (
            ...
            f's.setProperty("--code-fg","{colors["code_fg_color"]}");'
        )
        GLib.idle_add(
            self._web_view.evaluate_javascript,
            js, -1, None, None, None, None,
